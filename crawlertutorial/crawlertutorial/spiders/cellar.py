...
class CellarSpider(scrapy.Spider):
    name = 'cellar'
    allowed_domains = ['cellar.com']
    start_urls = ['https://cellar.com/vintages_wine.aspx']

    def parse(self, response):
        for link in response.css("table#mainContent_dlLinks tr td a::attr(href)"):
            yield response.follow(link.get(), callback=self.parse_vintages)

    
    def parse_vintages(self, response):
        counter = 0
